...
<body>
<form name="memberViewForm" action="${pageContext.request.contextPath}/member/updateMemberInfo.do" method="post">
...
<script type="text/javascript">
	$(function(){
		...
		//[1] =01
		//[2] =15
		$('input[name=mem_bir1]').val('${fn:split(memberInfo.mem_bir,"-")[0]}');
		$('input[name=mem_bir2]').val('${fn:split(memberInfo.mem_bir,"-")[1]}');
		$('input[name=mem_bir3]').val('${fn:split(memberInfo.mem_bir,"-")[2]}');
		...
		// 042-111-1111
		var comtel = '${memberInfo.mem_comtel}';
		...
		var email = '${memberInfo.mem_mail}';
		$('input[name=mem_mail1]').val(email.split('@')[0]);
		$('select[name=mem_mail2] option[value="'+email.split('@')[0]+'"]').attr('selected',true);
		
		$('input[name=mem_zip1]').val('${fn:split(memberInfo.mem_zip,"-")[0]}');
		$('input[name=mem_zip2]').val('${fn:split(memberInfo.mem_zip,"-")[0]}');
		...
		$('form[name=memberViewForm]').submit(function(){
			...
			$(this).append('<input type="hidden" name="mem_id" value="${memberInfo.mem_id}"></input>');
			...
			var hpt =$('select[name=mem_hp1]').val() +'-' + $('input[name=mem_hp2]').val() + '-' + $('input[name=mem_hp3]').val();
			...
			if(!$('input[name=mem_mail]').val().validationEMAIL()){
		...
		$('#btn3').click(function(){
			...
			var $frm =$('<form method="POST" action="${pageContext.request.contextPath}/member/deleteMember.do"></form>');
